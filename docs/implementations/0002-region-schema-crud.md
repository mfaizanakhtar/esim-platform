...
**Shipped:** 2026-04-26 · **PRs:** #226 (schema + CRUD), #227 (discovery), #228 (template generation), #229 (region-aware mapping), #230 (dashboard parity), #231 (discovery filter hotfix), #232 (FiRoam countryCodes normalization), #_TBD_ (smarter group labeling)
...
- **Discovery filters by `countryCodes.length >= 2`, NOT by `region IS NOT NULL`.** Provider sync code is inconsistent with the `region` field: TGT hard-codes `region: null` (admin.ts:4394, 4414), FiRoam stores the per-country code there (admin.ts:4216, 4235). Filtering by `region` would have excluded all TGT regional plans and grouped FiRoam single-country plans as "regions". The country list is the universal regional signal; group label comes from `parsedJson.regionCodes[0]` (preferred) → vendor `region` (if multi-char or in canonical aliases) → `MULTI-N` synthetic.
- **`/regions` page does NOT auto-fetch suggestions on mount.** Discovery scans the entire catalog and computes per-provider intersections — explicit "Discover suggestions" button keeps the work intentional. `useRegionSuggestions()` uses `enabled: false` and is triggered via `refetch()`. Saved regions list keeps auto-loading because it's a small static query.
