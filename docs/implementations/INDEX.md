...
| 0004 | Resend-email hydrates eSIM details | shipped | `POST /admin/deliveries/:id/resend-email` now looks up the SKU mapping and renders the same `productName / region / dataAmount / validity` box as the original delivery email; helper extracted so resend and provision share one derivation | [0004-resend-email-mapping-metadata.md](0004-resend-email-mapping-metadata.md) |
